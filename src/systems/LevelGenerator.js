import { GAME, BRICK, JARDINAIN } from '../config/Constants.js';
import { PAL } from '../config/Palette.js';
import { rand, clamp } from '../utils/Helpers.js';

// Tactical, Arkanoid-flavoured layouts: colored rows, pyramids, fortresses with
// indestructible gold pillars, silver armour, explosives, and nest bricks that
// house Jardinains. Returns brick descriptors { x, y, w, h, type, color }.

const PATTERNS = ['rows', 'pyramid', 'diamond', 'columns', 'checker', 'arch'];

export function buildLevel(level) {
  const isBoss = level % GAME.BOSS_EVERY === 0;
  const arenaLeft = GAME.WALL_X + BRICK.GAP;
  const arenaRight = GAME.WIDTH - GAME.WALL_X - BRICK.GAP;
  const arenaW = arenaRight - arenaLeft;
  const top = GAME.WALL_TOP + BRICK.GAP * 2;

  const cols = Math.max(6, Math.floor((arenaW + BRICK.GAP) / (BRICK.WIDTH + BRICK.GAP)));
  const bw = (arenaW - (cols - 1) * BRICK.GAP) / cols;
  const bh = BRICK.HEIGHT;
  const rows = isBoss ? 8 : clamp(4 + Math.floor(level / 2), 4, 8);

  const pattern = isBoss ? 'fortress' : PATTERNS[(level - 1) % PATTERNS.length];

  const silverChance = clamp(0.05 + level * 0.025, 0, 0.32);
  const explodeChance = 0.05;
  let nestBudget = Math.min(JARDINAIN.MAX_ALIVE, 1 + Math.floor(level / 2));

  const bricks = [];
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      if (!exists(pattern, r, c, rows, cols)) continue;

      let type = 'normal';
      let color = PAL.brickRows[r % PAL.brickRows.length];

      // structural indestructible gold
      if (isGold(pattern, r, c, rows, cols, level)) { type = 'gold'; color = PAL.gold; }
      else {
        const roll = Math.random();
        if (roll < explodeChance) { type = 'explosive'; color = PAL.explosive; }
        else if (roll < explodeChance + silverChance) { type = 'silver'; color = PAL.silver; }
        else if (nestBudget > 0 && Math.random() < 0.12) { type = 'nest'; color = PAL.nest; nestBudget--; }
      }

      bricks.push({
        x: arenaLeft + c * (bw + BRICK.GAP),
        y: top + r * (bh + BRICK.GAP),
        w: bw, h: bh, type, color,
      });
    }
  }

  // Guarantee a winnable field (at least a few destructibles).
  if (!bricks.some((b) => b.type !== 'gold')) {
    bricks.forEach((b) => { if (Math.random() < 0.4) { b.type = 'normal'; b.color = PAL.brickRows[0]; } });
  }
  return { bricks, isBoss };
}

function exists(pattern, r, c, rows, cols) {
  const midC = (cols - 1) / 2;
  switch (pattern) {
    case 'rows':
    case 'fortress':
    case 'full':
      return true;
    case 'pyramid':
      return Math.abs(c - midC) <= (rows - 1 - r) + 0.5;
    case 'diamond': {
      const midR = (rows - 1) / 2;
      return Math.abs(c - midC) / midC + Math.abs(r - midR) / midR <= 1.05;
    }
    case 'columns':
      return c % 3 !== 1;
    case 'checker':
      return (r + c) % 2 === 0;
    case 'arch':
      return !(r < rows - 2 && Math.abs(c - midC) < cols * 0.18);
    default:
      return true;
  }
}

function isGold(pattern, r, c, rows, cols, level) {
  if (level < 2) return false;
  if (pattern === 'fortress') {
    // gold border + a couple of inner pillars
    if (r === 0 || c === 0 || c === cols - 1) return true;
    if ((c === Math.floor(cols * 0.33) || c === Math.floor(cols * 0.66)) && r % 2 === 0) return true;
    return false;
  }
  if (pattern === 'columns' && c % 3 === 0 && r === Math.floor(rows / 2)) return true;
  return false;
}
